...
## 15. 검수 PDF 1차 수정 반영 (2026-07-10, 260710_수정체크_1)
- **3p(c1-map)**: `.map-badge`에 누락된 라운드 박스(알약) 추가 — 그라데이션 배경+radius22+shadow (app.css).
- **퀴즈 인물 사진(36/55p 등 전체)**: `.quiz-photo`에 `object-position: center top` — 얼굴이 보이도록 Top 정렬 (app.css).
- **sitemap 좌표/폰트 일괄 수정(22~26,28,31,32,35,40,41,44,51,52,54,59~71p)**: PDF 지시대로 단락 y/폰트 size/추가자료 버튼 rect 이동. 41p는 px 미지정이라 s2 y580/s3 y860로 추정 적용(검수 필요).
- **59p**: 타이틀 45px·y185, 배지 텍스트 y126 + **`badgeTop` 키 신설**(sitemap 페이지 속성 → renderFrame에서 frm-badge top 인라인 오버라이드, app.js). 059.json 타이틀 '…의지를 알리고,' 뒤 줄바꿈(\n, PDF의 '바꾸고'는 '알리고'의 오기로 판단).
- **미반영(사용자 확인 필요)**: 40p 코멘트의 '타이틀 내용 초과 시 폰트 자동 축소 기능' — 스펙 확정 후 구현.
- 검증: node --check OK · CSS 중괄호 균형 · 75 pages · NUL 0. 쓰기는 전부 bash+fsync+별도 프로세스 재검증.
## 16. 목차 팝업 구현 (2026-07-10)
- **우상단 '설정' → '목차' 교체**: sitemap chrome.top action `settings`→`toc`(label '목차'), ICONS에 리스트 아이콘 추가, handle()의 settings 케이스 제거. 퍼가기는 좌측 레일 유지.
- **목차 팝업**(app.js TOC_DATA + openToc/buildToc/closeToc, app.css `.toc-*`): 딤(rgba(15,15,35,.62), z-50, stage 내부) + 좌측 패널(155,15,640x1050, 네이비 그라데이션). **서랍식 아코디언** — 기본 4개 코스 노출, 셰브론으로 1개씩 전개.
- **연결 규칙**('목차 리스트.pptx' 기준): 코스 헤더→각 map(3/21/39/58p) · 사전조사→prestudy · 그룹 헤더(상하이 시기 등 12개)→해당 개론(intro) · ①~⑪ 세부 항목→읽기자료(read). 퀴즈는 pptx 목차에 없어 미포함.
- 열 때 현재 페이지의 코스 자동 전개 + 현재 항목 하이라이트(.cur). 닫기: X버튼/딤 클릭/ESC. overlayOpen()에 `.toc-dim.show` 추가(휠·방향키 차단). 페이지 2(toc 타입, 코스 카드)는 별개로 유지.
- pptx 오기 정정: '용인 거주지'→'요인 거주지', '워싱텅'→'워싱턴'.
- 검증: node --check OK · CSS 중괄호 균형 · TOC 링크 60개 전부 sitemap id 존재 · 75 pages · NUL 0.
## 17. 목차 장애 수정 (2026-07-10)
- **원인 1(치명)**: app.css 끝에 이전 truncation 사고 잔재 `.map-close:h`(중괄호 없는 조각)가 커밋돼 있었음. 마지막 줄일 땐 무해했으나 §16에서 그 뒤에 CSS를 추가하자 파서가 조각+`.toc-dim{...}`을 한 규칙으로 먹어 **`.toc-dim` 기본 규칙만 무효화** → 팝업이 position:static으로 카드 아래 그려지고, 닫기 불가로 `.show`가 남아 휠/방향키 차단. → `.map-close:hover { background: #c9c9d4; }`로 복원(a927b3c 원본).
- **원인 2**: index.html이 app.css/app.js를 버전 쿼리 없이 참조 → 브라우저 캐시로 구버전 CSS 서빙 가능. → `?v=20260710` 추가(이후 css/js 수정 시 이 값도 올릴 것 — app.js 내부 V와 별개).
- **보강**: go()에서 페이지 이동 시 closeToc() 자동 호출(팝업 열린 채 페이저 이동해도 딤 잔류 없음).
- 브라우저 실검증 완료: 2p 카드 위 팝업 표시/아코디언/항목 클릭 이동(25p)/이동 후 방향키 정상/재개방 시 현재 항목 하이라이트+코스 자동 전개/ESC·딤 닫기.
- 교훈: CSS 무결성 검사에 중괄호 균형만으론 부족 — '중괄호 없는 선택자 조각' 탐지 추가할 것.
## 18. 세부 조정 5건 (2026-07-10)
- **⚠ 작업 시작 시 sitemap.json 작업트리 사본이 또 truncation 상태**(HEAD 커밋본은 정상)여서 `git show HEAD:` 내용으로 복원 후 작업. `git checkout --`은 마운트에서 unlink 불가로 실패 → python 덮어쓰기 방식 사용. 또한 다수 파일에 CRLF 변환 diff가 보임(GitHub Desktop 영향, 내용 동일·무해).
- 2p 코스 카드 클릭 → 각 c1~c4-map 이동(01_title.js의 card id `courseN` 파싱, renderCard의 c1-map 하드코딩+toast 제거).
- 37/56/72p 퀴즈 photoSlot rect 604×408로 변경(x,y 유지).
- 40p: 갤러리 1장(photos.json 040 gallery에서 040_01.jpg 제거 — 원본 파일은 사용자가 커밋으로 삭제) · body 폰트 25→24("25 우선 적용" 아님, sitemap 값이 25였던 것).
- 목차: 열 때마다 접힌 기본 상태(현재 코스 자동 전개 제거, 스크롤 top 리셋, 현재 항목 .cur는 유지).
- index.html 캐시버스터 `?v=20260710b`로 증가(js/css 수정 시마다 올릴 것).
- 브라우저 검증: 카드→39p 이동, 목차 재개방 시 접힘, 40p body 24px/타이틀 47px/갤러리 1·1 화살표 숨김, 37p 사진 604×408+top 정렬.
## 19. 퀴즈 이미지 재조정 + pager 직접 입력 (2026-07-10)
- 37/56/72p photoSlot rect = **x260, y479, w715, h491** (§18의 604×408에서 재변경).
- **pager 페이지 직접 입력**: index.html `#pagerPos` span → `#pagerInput`(input) + `#pagerTotal`. Enter/blur로 이동, 숫자만 허용, 범위 밖·비숫자는 현재 페이지로 원복, Esc 원복, focus 시 전체 선택. 전역 방향키 핸들러는 INPUT 포커스 시 기존처럼 무시됨. CSS `.pager .pg-in`.
- 캐시버스터 `?v=20260710c`.
- 브라우저 검증: 37 입력 이동+사진 rect 적용, 999 입력 원복, 방향키 이동 시 입력값 동기(38).
