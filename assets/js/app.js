(function () {
  ...
  function renderTextBlocks(page, root, content){
    ...
    (page.textBlocks||[]).forEach(b=>{
      ...
      const n=el('div','txt-block',{left:px(b.x),top:px(b.y),width:px(b.w),fontSize:px(b.size),
  ...
  function renderQuiz(page, root, content){
    ...
    // 문제 텍스트 (편집 가능)
    if(page.qtext && q.question){ const c=page.qtext;
    ...
    if(page.photoSlot && pd.display[0]){ const rc=page.photoSlot.rect;
  ...
  function idb(){return new Promise((res,rej)=>{ if(_ndb)return res(_ndb); const r=indexedDB.open(NDB,1);
  ...
  function noteIndexFromPoint(q,x,y){
    try{ const rng=document.caretRangeFromPoint&&document.caretRangeFromPoint(x,y);
      if(rng){ const blot=Quill.find(rng.startContainer,true); if(blot&&blot.offset){ return blot.offset(q.scroll)+(rng.startOffset||0); } } }catch(e){}
    const sel=q.getSelection(); return sel?sel.index:q.getLength();
  }
  function renderScrapGrid(grid){
    idbAll('scraps').then(list=>{ SCRAPS=list; grid.innerHTML='';
      ...
      list.slice().reverse().forEach(s=>{ const cell=el('div','scrap-cell'); cell.draggable=true; cell.dataset.id=s.id; cell.title='끌어다 노트에 넣기 (더블클릭으로도 삽입)';
  ...
  function setupNoteImages(q, edc, card){
    ...
    function place(){ if(!selImg||!selImg.isConnected){ ov.style.display='none'; return; }
    ...
    function startCrop(){ if(!selImg)return; const sc=scaleOf();
      ...
      function mv(e){ if(!mode)return; const dx=(e.clientX-start.x)/sc, dy=(e.clientY-start.y)/sc;
        if(mode.move){ bx=Math.min(Math.max(ix,start.bx+dx), ix+iw-bw); by=Math.min(Math.max(iy,start.by+dy), iy+ih-bh); }
        else { const p=mode.resize; let nx=start.bx,ny=start.by,nw=start.bw,nh=start.bh;
      ...
      ok.addEventListener('click',()=>{ const nat=selImg.naturalWidth, natH=selImg.naturalHeight;
    ...
    eEl.addEventListener('keydown',e=>{ const ctrl=e.ctrlKey||e.metaKey;
  ...
  function renderNote(page, root){
    ...
    const q=new Quill(edc,{ theme:'snow', placeholder:'이번 탐방에서 보고 느낀 점을 자유롭게 작성하세요. 오른쪽 스크랩 이미지를 끌어다 넣을 수 있어요.',
    ...
    pdf.addEventListener('click',()=>{ if(!window.html2pdf){ toast('PDF 모듈 로딩 중입니다'); return; }
